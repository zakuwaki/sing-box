...
func NewVMess(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.VMessOutboundOptions) (*VMess, error) {
	...
	var err error
	if options.TLS != nil {
		outbound.tlsConfig, err = tls.NewClient(options.Server, common.PtrValueOrDefault(options.TLS))
		if err != nil {
			return nil, err
		}
	}
	if options.Transport != nil {
		var stdTLSConfig *tls.STDConfig
		if outbound.tlsConfig != nil {
			stdTLSConfig, err = outbound.tlsConfig.Config()
			if err != nil {
				return nil, err
			}
		}
		outbound.transport, err = v2ray.NewClientTransport(ctx, outbound.dialer, outbound.serverAddr, common.PtrValueOrDefault(options.Transport), stdTLSConfig)
		if err != nil {
